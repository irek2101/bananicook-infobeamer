...
-- colored texture cache
local tex_cache = {}
local function rect_col(x1,y1,x2,y2, col, a)
  local key = string.format("%.3f,%.3f,%.3f", col[1], col[2], col[3])
  local tex = tex_cache[key]
  if not tex then
    tex = resource.create_colored_texture(col[1], col[2], col[3], 1.0)
    tex_cache[key] = tex
  end
  tex:draw(x1, y1, x2, y2, a or 1.0)
end
...
local LABEL_BG = {0.24,0.12,0.12}
local HEADER_COL = {1.0, 0.88, 0.51}
...
local function draw_header(alpha)
  if not has_fonts() then return end
  rect_col(80,140, W-80,144, HEADER_COL, alpha)
  FONT_H1:write(80, 60, "BANANICOOK • MENU", 70, 1,1,1, alpha)
end

local function draw_category(title, col, alpha)
  if not has_fonts() then return end
  FONT_CAT:write(100, 170, title, 56, 1,1,1, alpha)
  rect_col(100, 235, W-100, 238, {col[1]*.7, col[2]*.7, col[3]*.7}, alpha)
end

local function write_price(price, y, col, alpha)
  if not has_fonts() then return end
  local size = 46
  local w = text_w(FONT_ITM, size, price)
  FONT_ITM:write(W-120-w, y, price, size, 1,1,1, alpha)
end

local function write_item(name, price, y, col, available, alpha)
  if not has_fonts() then return end
  local size = 46
  local c = available and col or MUTED
  FONT_ITM:write(120, y, name, size, 1,1,1, alpha)
  write_price(price, y, c, alpha)
  if not available then
    local w = text_w(FONT_ITM, size, name)
    rect_col(120, y+size*0.55, 120+w, y+size*0.58, {1, .5, .5}, alpha)
    local lbl = "brak dziś"
    local lw = text_w(FONT_SUB, 34, lbl)
    rect_col(120+w+16, y+8, 120+w+16+lw+20, y+size-6, {0.24,0.12,0.12}, alpha)
    FONT_SUB:write(120+w+24, y+8, lbl, 34, 1,1,1, alpha)
  end
end

local function render_category(cat, alpha, clear_bg)
  if clear_bg ~= false then gl.clear(0.07,0.08,0.09,1) end
  local bg = get_bg(cat.bg); if bg then bg:draw(0,0,W,H, 0.45*(alpha or 1)) end
  draw_header(alpha or 1)
  local col = palette[(1 + math.floor(sys.now()*10)) % #palette + 1]
  draw_category(cat.title, col, alpha or 1)
  local y = 250
  local idx = 1
  for _,it in ipairs(cat.items) do
    local show = cfg[it.key]; if show == nil then show = true end
    local c = palette[(idx-1)%#palette + 1]
    write_item(it.name, it.price, y, c, show, alpha or 1)
    y = y + 74
    idx = idx + 1
  end
  if LOGO then LOGO:draw(W-260, 40, W-60, 220, alpha or 1) end
  if has_fonts() then
    FONT_FTR:write(W/2-500, H-70, "1080p • Ceny w PLN • 'brak dziś' sterujesz w konfiguracji", 30, 1,1,1, alpha or 1)
  end
end
...
function node.render()
  ...
  if fade > 0 then
    ...
    render_category(nextcat or DATA[1], math.min(fade,1.0), false)
  end
end
